...
if (isset($_POST['search_submit'])) {
    ...
    // Query ke database untuk mencari data dari tabel "books"
    $query_books = "SELECT id, title, author, price, image FROM books WHERE title LIKE '%$searchQuery%' OR author LIKE '%$searchQuery%'";
    $result_books = mysqli_query($koneksi, $query_books);

    // Query ke database untuk mencari data dari tabel "buku_promo"
    $query_buku_promo = "SELECT id, title, author, price, image FROM buku_promo WHERE title LIKE '%$searchQuery%' OR author LIKE '%$searchQuery%'";
    $result_buku_promo = mysqli_query($koneksi, $query_buku_promo);

    // Tampilkan hasil pencarian dari tabel "books"
    if (mysqli_num_rows($result_books) > 0) {
        while ($row = mysqli_fetch_assoc($result_books)) {
            if (!in_array($row['title'], $displayedTitles)) {
                ...
                $displayedTitles[] = $row['title']; // Add title to the displayedTitles array
            }
        }
    } else {
        echo '<p </p>';
    ...
    // Tampilkan hasil pencarian dari tabel "buku_promo"
    if (mysqli_num_rows($result_buku_promo) > 0) {
        while ($row = mysqli_fetch_assoc($result_buku_promo)) {
            if (!in_array($row['title'], $displayedTitles)) {
                echo '<div class="result-item">';
                echo '<br>';
                $imagePath = '../' . $row['image']; // Menambahkan "../" sebelum path gambar
                echo '<a href="promo.php?id=' . $row['id'] . '"><img src="' . $imagePath . '" alt="Book Image"></a>';
        ...
        echo '<p class="no-result">Tidak ada hasil yang ditemukan di .</p>';
    }
}
?>
